...
class Obstacle:
    ...
    def get_robot_target_pos(self):
        ...
        if self.pos.direction == Direction.TOP:
            pos =  RobotPosition(self.pos.x, self.pos.y + settings.OBSTACLE_SAFETY_WIDTH + settings.OBSTACLE_LENGTH + 30, Direction.BOTTOM)
        elif self.pos.direction == Direction.BOTTOM:
            pos = RobotPosition(self.pos.x, self.pos.y - settings.OBSTACLE_SAFETY_WIDTH - settings.OBSTACLE_LENGTH - 30, Direction.TOP)
        elif self.pos.direction == Direction.LEFT:
            pos = RobotPosition(self.pos.x - settings.OBSTACLE_SAFETY_WIDTH - settings.OBSTACLE_LENGTH - 30, self.pos.y, Direction.RIGHT)
        else:
            pos = RobotPosition(self.pos.x + settings.OBSTACLE_SAFETY_WIDTH + settings.OBSTACLE_LENGTH + 30, self.pos.y, Direction.LEFT)
